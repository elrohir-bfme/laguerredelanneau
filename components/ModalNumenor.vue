    ...
    <div class="z-40 bg-white rounded-lg overflow-hidden shadow-xl transform transition-all" role="dialog"
      ...
      <div class="max-w-2xl rounded overflow-hidden shadow-lg">
        <div class="relative flex">
          <img v-if="region" :class="region.minimap ? 'w-2/3' : 'w-full'" :src="`https://api.laterredumilieu.fr${region.img.url}`">
          <div v-if="region && region.minimap" class="w-1/3 flex flex-col">
            <img v-if="region" class="object-contain" :src="region.minimap && `https://api.laterredumilieu.fr${region.minimap.url}`">
            ...
            <span  v-if="region && region.land" class="absolute bottom-2 left-2 text-white text-xl shadow-md underline inline-flex items-center">
        ...
        <div class="px-6 py-3">
          <div class="flex justify-between font-bold text-xl mb-2">
            <div v-if="region">
              <p @click="copyId()" :class="region && region.minimap && 'absolute top-2 left-2'" class="bg-gray-700 hover:bg-gray-800 text-white font-bold py-2 ml-2 px-4 rounded text-base">
                {{region.name}}
              </p>
            </div>

            <!-- <div v-if="region && region"
              class="bg-gray-800 hover:bg-gray-900 text-white font-bold py-2 px-3 rounded-full text-base">
              Map {{region.map}} Joueurs
            </div> -->
            <div>
              <a v-if="region && region.mapdl && !region.minimap" target="_blank" :href="region.mapdl"
        ...
        <div v-if="region && region.adjacents && region.adjacents.length > 0" >
          <div>
            <div>
            ...
            <div class="p-2 text-center" v-show="active" v-for="adjacent in region.adjacents" :key="adjacent"><slot />
             <button @click="ActionAdjacent(adjacent)">{{ adjacent }}</button> 
            </div>            
          </div>
        </div>
